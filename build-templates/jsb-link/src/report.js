...
window.boot = function () {
    ...
    var uid = wx.getStorageSync("guid")
    // 初始化开始
    wx.report("闪屏-1.1引擎加载-" + (uid ? "false" : "true"))
    ...
    var preBoot = function (bName) {
        cc.log("bundle:", bName)
        var dir = jsb.fileUtils.getWritablePath() + "bundles/assets/" + bName + "/"
        var fileList = jsb.fileUtils.listFiles(dir)
        var useLocal = true
        var md5Cache = null
        for (var j = 0; j < fileList.length; j++) {
            var fname = fileList[j]
            var name = fname.substring(dir.length)
            if (/^config.(\w*\.*)*json$/.test(name)) {
                var strArr = name.split(".")
                md5Cache = strArr[1]
            } else if (name === "project.manifest") {
                try {
                    var flag = null
                    var jsonCache = JSON.parse(jsb.fileUtils.getStringFromFile(fname))
                    var jsonLocal = JSON.parse(jsb.fileUtils.getStringFromFile("manifest/" + bName + "/project.manifest"))
                    var vCache = jsonCache["version"]
                    var vLocal = jsonLocal["version"]
                    cc.log("vCache:", vCache, "vLocal:", vLocal)
                    var vC = vCache.split('.');
                    var vL = vLocal.split('.');
                    for (var i = 0; i < vC.length; ++i) {
                        var c = parseInt(vC[i] || "0") || 0;
                        var l = parseInt(vL[i] || "0") || 0;
                        if (c === l) {
                            continue;
                        }
                        else {
        ...
        if (!useLocal) {
            if (md5Cache !== null) {
                settings.bundleVers[bName] = md5Cache
            }
        }
    }
    for (var b in settings.bundleVers) {
        preBoot(b)
    }

    var searchPaths = jsb.fileUtils.getSearchPaths();
    Array.prototype.unshift.apply(searchPaths, [jsb.fileUtils.getWritablePath() + "bundles/"]);
    ...
    var onStart = function () {
        ...
        var launch = function () {
            wx.report("闪屏-1.4loading界面资源加载-" + (uid ? "false" : "true"))
            bundle.loadScene(launchScene, null, onProgress,
                function (err, scene) {
                    if (!err) {
                        wx.report("闪屏-1.4.1loading界面资源加载结束-" + (uid ? "false" : "true"))
    ...
    function cb(err) {
        ...
        if (count === bundleRoot.length + 1) {
            cc.assetManager.loadBundle(MAIN, function (err) {
                if (!err) {
                    wx.report("闪屏-1.3.1加载脚本和启动包完成-" + (uid ? "false" : "true"))
                    cc.game.run(option, onStart);
                } else {
                    wx.report("闪屏-1.3.2加载启动包失败-" + (uid ? "false" : "true"))
                }
            });
        }
    }

    wx.report("闪屏-1.3加载脚本和启动包-" + (uid ? "false" : "true"))
...
! function () {
    ...
    var R = {
        aldSendEvent: function (e, t) {
            var n = g(I);
            n.g0 = "newbiew-" + (!wx.getStorageSync("guid") || wx.getStorageSync("newbiew")).toString();
...
if (result.game && result.game[0] && result.game[0].PacketName) {
    igsgame.igsChannel = result.game[0].PacketName
}
console.log("igs channel", JSON.stringify(igsgame.igsChannel))
