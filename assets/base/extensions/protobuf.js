    ...
    ProtoBuf.Reflect = (function(ProtoBuf) {
        ...
        function mkLong(value, unsigned) {
            if (value && typeof value.low === 'number' && typeof value.high === 'number' && typeof value.unsigned === 'boolean'
                ...
                return new ProtoBuf.Long(value.low, value.high, typeof unsigned === 'undefined' ? value.unsigned : unsigned).toNumber();
            if (typeof value === 'string')
                return ProtoBuf.Long.fromString(value, unsigned || false, 10).toNumber();
            if (typeof value === 'number')
                return ProtoBuf.Long.fromNumber(value, unsigned || false).toNumber();
                ...
                var Message = function(values, var_args) {
                    ...
                    if (arguments.length > 0) {
                        ...
                        if (T.syntax !== 'proto3') {
                            for (i=0, k=fields.length; i<k; ++i) {
                                var field = fields[i];
                                if (this[field.name] == null && field.required) {
                                    this[field.name] = Element.defaultFieldValue(field.type);
                                    var emsg = "one required field for: " + field + " is missing. set value with " + this[field.name];
                                    console.warn ? console.warn(emsg) : console.log(emsg);
                                }
                            }
                        }
                    }
